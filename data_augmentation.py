...
def apply_data_agumentation_to_train(train_path):
    ...
    for img_path in images:
        ...
        mask_name = mask.split('.')[1].split('/')[-1]
        ...
        mask_h_flip.save(f"{train_path}/masks/{mask_name}_h_flip.png")

        #random vertical flip
        image_v_flip = TF.vflip(image)
        mask_v_flip = TF.vflip(mask)
        with tifffile.TiffWriter(f"{train_path}/{image_name}_v_flip.tif.png") as tif:
            tif.write(image_v_flip.numpy())
        mask_v_flip.save(f"{train_path}/masks/{mask_name}_v_flip.png")
        ...
        mask_rotated.save(f"{train_path}/masks/{mask_name}mask_rotated.png")
